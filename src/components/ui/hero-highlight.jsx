...
export const HeroHighlight = ({ children, className, containerClassName }) => {
  ...
  return (
    ...
    >
      {/* light mode dots */}
      <div
        className="absolute inset-0 pointer-events-none opacity-60 dark:opacity-0"
        style={dotPattern("hsl(var(--muted-foreground) / 0.35)")}
      />
      {/* dark mode dots */}
      <div
        className="absolute inset-0 pointer-events-none opacity-0 dark:opacity-60"
        style={dotPattern("hsl(var(--muted-foreground) / 0.5)")}
      />
      {/* hover highlight */}
      <motion.div
...
export const Highlight = ({ children, className }) => {
  return (
    <motion.span
      ...
      className={cn(
        "relative inline-block pb-1 px-1 rounded-md bg-foreground/10 dark:bg-foreground/20",
        className
      )}
    >
      {children}
